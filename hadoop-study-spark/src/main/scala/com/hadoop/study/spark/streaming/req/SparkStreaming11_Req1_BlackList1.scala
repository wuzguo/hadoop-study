...
object SparkStreaming11_Req1_BlackList1 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val sparkConf = new SparkConf().setMaster("local[*]").setAppName("SparkStreaming11_Req1_BlackList1")
                    ...
                    case ( ( day, user, ad ), count ) =>
                        ...
                        if ( count >= 30 ) {
                            ...
                            val sql = """
                                        ...
                                        |UPDATE user_id = ?
                                      """.stripMargin
                            JDBCUtil.execute(conn, sql, Array( user, user ))
                            conn.close()
                        } else {
                            ...
                            val sql = """
                                      ...
                                      """.stripMargin
                            val flg = JDBCUtil.query(conn, sql, Array( day, user, ad ))
                            ...
                            if ( flg ) {
                                ...
                                val sql1 = """
                                             ...
                                             | where dt = ? and user_id = ? and ad_id = ?
                                           """.stripMargin
                                JDBCUtil.execute(conn, sql1, Array(count, day, user, ad))
                                ...
                                val sql2 = """
                                           ...
                                           """.stripMargin
                                val flg1 = JDBCUtil.query(conn, sql2, Array( day, user, ad ))
                                if ( flg1 ) {
                                    val sql3 = """
                                                |insert into black_list (user_id) values (?)
                                                |on DUPLICATE KEY
                                                |UPDATE user_id = ?
                                              """.stripMargin
                                    JDBCUtil.execute(conn, sql3, Array( user, user ))
                                }
                            } else {
                                val sql4 = """
                                             | insert into user_ad_count ( dt, user_id, ad_id, count ) values ( ?, ?, ?, ? )
                                           """.stripMargin
                                JDBCUtil.execute(conn, sql4, Array( day, user, ad, count ))
                            }
                            conn.close()
                        }
                }
