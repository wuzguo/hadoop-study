...
public abstract class UtilJson {
    ...
    public static <T> T readValue(String json, Class<?> parametrized, Class<?> parametersFor,
        ...
        if (parameterClasses == null || parameterClasses.length == 0) {
            type = mapper.getTypeFactory().constructParametricType(parametrized, parametrized, parametersFor);
        } else {
    ...
    /**
     * 转换为目标类，如果value是字符串，将被认为是json串
     * <br> 所以特别注意：'"abc"'是json字符串，目标类型是String时，转换结果为'abc'而不是'"abc"'<br>
     *
