...
import com.hadoop.study.spark.recomnend.beans.ProductRecs
import com.hadoop.study.spark.recomnend.config.MongoConfig
import com.hadoop.study.spark.recomnend.utils.ConnHelper
...
object OnlineRecommender {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val simProductsMatrix = spark.read
          ...
          .map { recs =>
              (recs.productId, recs.recs.map(rec => (rec.productId, rec.score)).toMap)
          }
          .collectAsMap()

        // 定义广播变量
        val simBcProductsMatrix = sc.broadcast(simProductsMatrix)
            ...
            rdds =>
                rdds.foreach {
                    case (userId, productId, score, timestamp) =>
                        ...
                        val candidateProducts = getTopSimProducts(MAX_SIM_PRODUCTS_NUM, productId, userId, simBcProductsMatrix.value)

                        // 3. 计算每个备选商品的推荐优先级，得到当前用户的实时推荐列表，保存成 Array[(productId, score)]
                        val streamRecs = computeProductScore(candidateProducts, userRecentlyRatings, simBcProductsMatrix.value)

                        // 4. 把推荐列表保存到mongodb
                        save(userId, streamRecs)
                }
        }
    ...
    // 写入mongodb
    def save(userId: Int, streamRecs: Array[(Int, Double)])(implicit mongoConfig: MongoConfig): Unit = {
        val collection = ConnHelper.mongoClient(mongoConfig.db)(STREAM_RECS)
        // 按照userId查询并更新
        collection.findAndRemove(MongoDBObject("userId" -> userId))
        collection.insert(MongoDBObject("userId" -> userId, "recs" -> streamRecs.map(x => MongoDBObject("productId" -> x._1, "score" -> x._2))))
    }
}
